...
import { getCandidateById } from "@features/recruiter/services/candidate.service";
import { getCandidateATSHistory } from "@features/recruiter/services/ats.candidateHistory.service";
import type { Candidate, CandidateStatus } from "@types/candidate";
import type { CandidateATSHistoryItem } from "@types/atsAnalysis";
...
export default function HRCandidateDetails() {
  ...
  const [atsAnalyses, setATSAnalyses] = useState<CandidateATSHistoryItem[]>([]);
  ...
  useEffect(() => {
    ...
    const loadCandidate = async () => {
      try {
        ...
        setError("Unable to load this candidate. Please try again.");
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const loadATSHistory = async () => {
      try {
        ...
        const analyses = await getCandidateATSHistory(candidateId);
        setATSAnalyses(analyses);
      } catch (err) {
        console.error("Failed to load candidate ATS history:", err);
        setATSError("Unable to load ATS analysis history.");
      } finally {
        setATSLoading(false);
      }
    };
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-3 text-xs text-slate-400">
                  Candidate since {formatCreatedDate(candidate.createdAt)}
                </p>
              </div>
            </div>
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                {atsAnalyses.length > 0 && (
                  <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
                    {atsAnalyses.length}{" "}
                    {atsAnalyses.length === 1 ? "analysis" : "analyses"}
                  </span>
                )}
              </div>
            </div>
            ...
            {atsLoading ? (
              ...
              <div className="divide-y divide-slate-100">
                {atsAnalyses.map((analysis) => (
                  <ATSHistoryRow key={analysis._id} analysis={analysis} />
                ))}
              </div>
            )}
          </section>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <InfoRow
                ...
                label="Resume"
                value={candidate.resume ? "Available" : "Not available"}
              />
            </div>
          </section>
...
function ATSHistoryRow({
  ...
  const score = Math.round(analysis.rankingScore ?? 0);
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-slate-500">
                {analysis.job?.company || "Unknown Company"}
              </p>
