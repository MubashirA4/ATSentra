...
import { getRecruiterResumeDetails } from "../../services/resume/recruiterResumeApi";
...
const HRResumeDetails = () => {
  ...
  const [resume, setResume] =
    useState<RecruiterResume | null>(null);
  ...
  useEffect(() => {
    ...
    const loadResume = async () => {
      try {
        ...
        const data =
          await getRecruiterResumeDetails(resumeId);

        setResume(data.resume);
        setResumeContent(
          data.resumeContent ?? null,
        );
      } catch (error) {
        console.error(error);
        toast.error(
  ...
  const experiences =
    (parsed?.experience as ExperienceItem[]) ??
    [];

  const education =
    (parsed?.education as EducationItem[]) ??
    [];
